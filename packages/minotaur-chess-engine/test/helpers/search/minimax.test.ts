...
import { maxMove, scoredMove } from "./minimax";
import { applyMove } from "../bitboards";
...
import { generateNodeId } from "./nodes/nodeGenerators";
...
describe("maxMove function", () => {
  it("picks a central pawn move from a starting board for white", () => {
    ...
    if (showEvalOutputs) {
      outputEvalLogsHtml(
        evalLogs,
        "picks a central pawn move from a starting board for white"
      );
    }
  });
  ...
  it.each([1, 2, 3, 4])("picks a valid move for white at %s depth", (depth) => {
    ...
    if (showEvalOutputs) {
      outputEvalLogsHtml(
        evalLogs,
        `picks a valid move for white at ${depth} depth`
      );
    }
  });
  ...
  it.each([1, 2, 3, 4])("picks a valid move for black at %s depth", (depth) => {
    ...
    if (showEvalOutputs) {
      outputEvalLogsHtml(
        evalLogs,
        `picks a valid move for black at ${depth} depth`
      );
    }
  });
  it("picks d5 from a starting board for black after white d4 at depth of 2", () => {
    ...
    if (showEvalOutputs) {
      outputEvalLogsHtml(
        evalLogs,
        "picks a central pawn move from a starting board for black"
      );
    }
  });
  ...
  it("includes a move to castle short for black when possible", () => {
    ...
    evalLogs.evalLayers.forEach((layer) => {
      const castledEval = layer.evaluations.find((evaluation) => {
        kingCastled =
          evaluation.boardState.blackKing ===
            CastleForBlackAfterGameBoard.blackKing &&
          evaluation.boardState.blackRook ===
            CastleForBlackAfterGameBoard.blackRook;
      });
