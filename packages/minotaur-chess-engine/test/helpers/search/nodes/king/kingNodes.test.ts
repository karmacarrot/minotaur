...
import { NodeFactory, StartingNode, generateNodeId } from "../nodeGenerators";
import { kingCastlingNodes, kingNodes } from "./kingNodes";
...
describe("kingNodes", () => {
  it("returns all legal white king nodes after executing a bong cloud", () => {
    ...
    outputEvalLogsHtml(
      evalLogs,
      "returns all legal white king nodes after executing a bong cloud"
    );
  ...
  it("does not make the black King charge the white Queen when checked(!!)", () => {
    ...
    const kingDsixMask = binaryMask64(
      kingDsixPosition,
      "all_zeroes_with_position_as_one"
    );

    expect(
      secondKingPositions.find((x) => x.boardState.blackKing === kingDsixMask)
    ).toBeNull;
  });
});
